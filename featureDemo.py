...
import os
import sys
sys.path.append(r'/Volumes/T7 Shield/TBME_code2.0/utils')
# import feature
import tremor_utils
import pd_utils
#%
# get feature
data_path= "/Volumes/T7 Shield/rename_PD_DATA_2/"
...
window_size=300
overlapping_rate = 2  #0.5 overlap, overlap = 1 / overlapping_rate
frequency = 200
pd_num = 100
# activity_num = 6
side_l = "wristl"
...
import pandas as pd
Feature = pd.read_csv(data_path + "feature_right_side.csv")

# 将修改后的 DataFrame 重新写入 CSV 文件
# df.to_csv(file_path, index=False)  # 这里的 index=False 表示不写入索引列
label_data = pd.read_excel(data_path + "Information_Sheet_Version_1.csv")
...
feature_label.to_csv(data_path + "m12activity_feature_label_right_level.csv", index=False)